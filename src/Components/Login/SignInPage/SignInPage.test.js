...
describe("Sign In Page", () => {
  ...
  test("Sign In static components testing", async () => {
    ...
    const welcomeMessage = await screen.getByText("Sign in to Twitter");
    expect(welcomeMessage).toBeInTheDocument();
    const googleSignIn = await screen.getByText("Sign in with Google");
    expect(googleSignIn).toBeInTheDocument();
    const appleSignIn = await screen.getByText("Sign in with Apple");
    expect(appleSignIn).toBeInTheDocument();
    const nextButton = await screen.getByText("Next");
    expect(nextButton).toBeInTheDocument();
    const forgotPassword = await screen.getByText("Forgot Password?");
